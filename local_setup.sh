...
#   ./local_setup.sh status           # read-only security/health dashboard:
#                                     #   hardened mode, LUKS, pins, drift, alerts
...
BASE_PACKAGES=(
  ...
  # eza: modern ls (icons + git status), aliased in .zshrc when present.
  # imagemagick: `convert` generates the blurred lock-screen wallpaper
  # (apply-theme.sh) and the i3 lock overlay.
  bat grc net-tools lm-sensors conky-all iproute2 eza imagemagick
...
deploy_phase() {
  ...
  case "$DESKTOP" in
    ...
    plasma)
      ...
      for f in kdeglobals kwinrc kwinrulesrc plasmarc konsolerc breezerc krunnerrc; do
...
# Read-only audit query — lets health.py attribute a touched monitored
# file to a comm/pid/uid via \`ausearch -k integrity\`.  ausearch only
# READS the audit log (no state change, no privilege escalation), so it's
# safe to NOPASSWD; scoped to the integrity key.
Cmnd_Alias DOTFILES_AUDITQ = \\
    /usr/sbin/ausearch -k integrity *

${USER} ALL=(root) NOPASSWD: DOTFILES_VPN, DOTFILES_APT, DOTFILES_SVC, DOTFILES_NET, DOTFILES_SECLOG, DOTFILES_AUDITQ
EOF
}
...
# Generate /etc/audit/rules.d/dotfiles-integrity.rules from the user's
# monitored-file manifest (~/.config/conky/.integrity-paths), one `-w`
# watch per file.  Kept SEPARATE (paths are user/$HOME-specific, so they
# can't live in the static dotfiles.rules) — removed on unharden.  This is
# what gives `ausearch -k integrity` the comm/pid/uid of whoever touches a
# monitored file, which health.py embeds into the alert.
_install_integrity_audit_rules() {
  local manifest="${HOME}/.config/conky/.integrity-paths"
  local tmp p
  local -a paths=()
  if [[ -f "$manifest" ]]; then
    while IFS= read -r p; do
      [[ "$p" =~ ^[[:space:]]*# ]] && continue   # comment
      [[ -z "${p//[[:space:]]/}" ]] && continue  # blank
      paths+=("${p/#\~/$HOME}")                   # expand leading ~
    done < "$manifest"
  else
    # Manifest not bootstrapped yet (conky hasn't run) — fall back to the
    # documented defaults so the watches still land.
    paths=("${HOME}/.aws/credentials.bak" "${HOME}/.ssh/id_rsa_old" \
           "${HOME}/Documents/passwords.txt")
  fi
  (( ${#paths[@]} )) || return 0
  tmp="$(mktemp)" || return 1
  {
    printf '# dotfiles file-integrity watches — generated by harden %s\n' "$(date -Iseconds)"
    printf '# Query: sudo ausearch -k integrity --start recent\n'
    for p in "${paths[@]}"; do
      printf -- '-w %s -p rwa -k integrity\n' "$p"
    done
  } > "$tmp"
  sudo install -D -m 0640 -o root -g root "$tmp" \
       /etc/audit/rules.d/dotfiles-integrity.rules
  local rc=$?
  rm -f "$tmp"
  return "$rc"
}
...
harden_auditd() {
  log "Configuring auditd rules (identity, sudoers, modules, mount, file-integrity)…"
  ...
  # File-integrity watches — generated from the user's monitored-file
  # manifest so `sudo ausearch -k integrity` attributes any access to a
  # UID/PID/comm.  The health.py polling check is the FAST alert; this is
  # the forensic "who touched it" layer (catches reads even on noatime).
  _install_integrity_audit_rules || warn "(file-integrity audit watches not added)"
...
unharden_auditd() {
  ...
  # Drop the generated file-integrity watches alongside the main ruleset.
  sudo rm -f /etc/audit/rules.d/dotfiles-integrity.rules 2>/dev/null || true
...
# --- harden orchestrator ------------------------------------
# World-readable marker recording that the system is in HARDENED mode.
# The narrow sudoers file is root-only (0440), so an unprivileged shell
# (zsh login banner, conky health.py, `status`) can't read it to infer
# mode.  This marker is the user-readable source of truth: written by
# harden_phase, removed by unharden_phase.  Format is simple key=value
# lines so both shell and python can parse it trivially.
HARDENED_MARKER="/etc/dotfiles-hardened"

_write_hardened_marker() {
  local measures="$1" tmp rc
  tmp="$(mktemp)" || return 1
  {
    printf 'hardened_at=%s\n' "$(date -Iseconds)"
    printf 'hardened_by=%s\n' "${USER}"
    printf 'measures=%s\n' "$measures"
  } > "$tmp"
  sudo install -m 0644 -o root -g root "$tmp" "$HARDENED_MARKER"
  rc=$?                 # capture BEFORE rm, else we'd return rm's status
  rm -f "$tmp"
  return "$rc"
}
...
harden_phase() {
  ...
  local measures=""
  harden_uu     && measures+="unattended-upgrades," || warn "(unattended-upgrades step had warnings)"
  harden_auditd && measures+="auditd,"              || warn "(auditd step had warnings)"
  harden_dot    && measures+="dot,"                 || warn "(DNS-over-TLS step had warnings)"
  harden_ufw    && measures+="ufw,"                 || warn "(ufw step had warnings)"
  harden_sudo   && measures+="narrow-sudo,"         || die  "(sudoers step failed — refusing to leave system half-hardened)"
  harden_seclog && measures+="seclog-tier2,"        || warn "(security-log Tier-2 step had warnings; Tier-1 user log still active)"
  _write_hardened_marker "${measures%,}" \
    || warn "(could not write ${HARDENED_MARKER}; status indicators may show install mode)"
  echo
  ok "Hardening complete."
  echo
  log "  what changed (old → new):"
  log "    sudo      broad NOPASSWD ALL  →  narrow DOTFILES_* allowlist"
  log "    firewall  none                →  ufw default-deny incoming"
  log "    DNS       NM plain resolvers  →  systemd-resolved opportunistic DoT"
  log "    updates   manual only         →  unattended security upgrades"
  log "    audit     none                →  auditd identity/sudoers/modules/mount"
  log "    seclog    user, tamper-evident→  root, append-only (tamper-resistant)"
  echo
  log "  from now on \`sudo <anything-not-allowlisted>\` will PROMPT for a password — that's the point."
  log "  verify:  sudo -l · sudo ufw status · resolvectl status · sudo auditctl -l"
  log "  status:  ./local_setup.sh status        revert: ./local_setup.sh unharden"
}

unharden_phase() {
  ensure_sudo
  warn "SECURITY: unharden re-enables broad NOPASSWD ALL (every command, not just VPN/apt),"
  warn "          disables ufw, auto-upgrades, auditd rules, and DoT.  Use only for re-provisioning."
  ...
  unharden_auditd
  unharden_dot
  sudo rm -f "$HARDENED_MARKER" 2>/dev/null
  echo
  ok "Hardening reverted (suitable for re-running setup)."
  warn "Remember to re-run \`./local_setup.sh harden\` once setup is complete."
}

# ── status — one-glance security/health dashboard ───────────
# Read-only aggregator so the six standalone tools (doctor, audit,
# verify-pins, seclog, hardening state) feel like one suite.  No sudo,
# no system changes.
status_phase() {
  local sec="════════════════════════════════════════════════════════"
  printf '\n%s\n  DOTFILES STATUS\n%s\n' "$sec" "$sec"

  # 1. Hardening mode — the headline "am I secure?" signal.
  echo
  if [[ -f "$HARDENED_MARKER" ]]; then
    local m at
    m="$(sed -n 's/^measures=//p' "$HARDENED_MARKER" 2>/dev/null)"
    at="$(sed -n 's/^hardened_at=//p' "$HARDENED_MARKER" 2>/dev/null)"
    ok "HARDENED  (${m:-?})"
    log "          since ${at:-unknown}"
  else
    warn "INSTALL MODE — broad sudo, no firewall, plain DNS."
    warn "  → harden when setup is stable:  ./local_setup.sh harden"
  fi

  # 2. Disk encryption (LUKS) — a hard prerequisite for the laptop.
  # `|| true`: findmnt exits non-zero when it can't resolve /, which
  # under `set -e` would otherwise abort the whole dashboard.
  local root_src
  root_src="$(findmnt -no SOURCE / 2>/dev/null || true)"
  if [[ -n "$root_src" ]] && lsblk -nso TYPE "$root_src" 2>/dev/null | grep -q crypt; then
    ok "disk encryption: LUKS active on /"
  else
    warn "disk encryption: / is NOT on a LUKS device (see readme/security.md → LUKS)"
  fi

  # File-integrity sentinels (read-only --status: lists state only).
  local fi_py="${HOME}/.config/conky/integrity.py"
  [[ -f "$fi_py" ]] || fi_py="${DOTFILES_DIR}/conky/integrity.py"
  if [[ -f "$fi_py" ]]; then
    local fi_out fi_armed fi_missing
    fi_out="$(python3 "$fi_py" --status 2>/dev/null || true)"
    fi_armed="$(printf '%s\n' "$fi_out" | grep -c '\[armed' || true)"
    fi_missing="$(printf '%s\n' "$fi_out" | grep -c '\[missing' || true)"
    if (( fi_missing > 0 )); then
      warn "file integrity: ${fi_armed} OK, ${fi_missing} MISSING — check seclog / ausearch -k integrity"
    else
      ok "file integrity: ${fi_armed} monitored file(s) intact"
    fi
  fi

  # 3. Supply-chain pins.  Capture first (verify-pins exits 1/2 on
  # stale/bad — expected — and `set -o pipefail` would otherwise make a
  # `| grep` pipeline report that as failure and fire the `|| ok`).
  echo; printf '%s SUPPLY CHAIN %s\n' "----" "------------------------------------"
  if [[ -x "${SCRIPT_DIR}/scripts/verify-pins.sh" ]]; then
    local vp
    vp="$(NO_COLOR=1 "${SCRIPT_DIR}/scripts/verify-pins.sh" 2>/dev/null || true)"
    if printf '%s\n' "$vp" | grep -qE '^\[(bad|stale)\]'; then
      printf '%s\n' "$vp" | grep -E '^\[(bad|stale)\]'
    else
      ok "all pins fresh + verified"
    fi
  fi

  # 4. Drift / baselines.  `|| true` because audit.sh now exits 1/2 on
  # warn/bad (a result, not a tool fault) — don't let it abort `status`.
  echo; printf '%s DRIFT %s\n' "----" "-------------------------------------------"
  if [[ -x "${SCRIPT_DIR}/scripts/audit.sh" ]]; then
    NO_COLOR=1 "${SCRIPT_DIR}/scripts/audit.sh" 2>/dev/null | tail -n 6 || true
  fi

  # 5. Recent security alerts (seclog) — prefer the deployed viewer.
  # Pipe (non-tty) → no colour → event rows start with the ISO date;
  # filter on that so the viewer's variable-length header is skipped
  # robustly (a rollover adds a header line, so a fixed `tail -n +N`
  # would be fragile).
  echo; printf '%s RECENT ALERTS %s\n' "----" "-----------------------------------"
  local seclog_py="${HOME}/.config/conky/seclog.py"
  [[ -f "$seclog_py" ]] || seclog_py="${DOTFILES_DIR}/conky/seclog.py"
  if [[ -f "$seclog_py" ]]; then
    local alerts
    alerts="$(python3 "$seclog_py" --tail -n 5 2>/dev/null | grep -E '^[0-9]{4}-[0-9]{2}-' || true)"
    if [[ -n "$alerts" ]]; then
      printf '%s\n' "$alerts"
    else
      log "(no security alerts logged yet)"
    fi
  fi
  echo
  log "detail:  ./scripts/dotfiles-doctor.sh   ·   seclog   ·   seclog -n 50"
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    setup|detect|install|deploy|terminal|validate|harden|unharden|status)
      ACTION="$1" ;;
    --show-overrides)
