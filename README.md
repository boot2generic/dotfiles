...
| `scripts/take-over-wifi.sh`           | Hands wifi from ifupdown / iwd / wpa_supplicant / systemd-networkd to NetworkManager — pre-imports SSID/PSK from `/etc/network/interfaces` into NM before stopping the old backend so reconnect is automatic. Makes the handover **persist across reboots** (comments out the ifupdown stanzas in `/etc/network/interfaces` *and* `interfaces.d/*`, writes the NM `conf.d` snippet, disables the old backends at boot) — without that, `ifup -a` reclaims the card every boot and you'd re-run the takeover forever; see readme/system.md → "I have to re-run the takeover after every reboot". If NM already owns the card it repairs just the persistent half, without touching the live link. Run by `local_setup.sh setup` automatically when applicable; can also be invoked by hand. `--revert` undoes the takeover (restores the most recent `/etc/network/interfaces.bak.<TS>`, removes the NM conf.d snippet, re-enables the prior backend). |
...
| `config/system/etc/environment.d/95-nvidia-wayland.conf`                     | same path under `/`                                        | `GPU_VENDOR=nvidia`               | D1 — sets `WLR_NO_HARDWARE_CURSORS=1`, `MOZ_ENABLE_WAYLAND=1`; VRR (`__GL_GSYNC_ALLOWED`/`__GL_VRR_ALLOWED`) is commented OFF by default (unstable with mixed-VRR multi-monitor on NVIDIA Wayland — uncomment for single/uniform-VRR setups). Read by `pam_systemd` at graphical-session start; logout/login required. |
