...
| Step | Where | Why |
|---|---|---|
| `/etc/environment.d/95-nvidia-wayland.conf` | `deploy_nvidia_wayland_env()` — drops `config/system/etc/environment.d/95-nvidia-wayland.conf` | Sets `WLR_NO_HARDWARE_CURSORS=1`, `MOZ_ENABLE_WAYLAND=1`; VRR (`__GL_GSYNC_ALLOWED`/`__GL_VRR_ALLOWED`) is commented OFF by default — a mixed-VRR multi-monitor mix (G-Sync + FreeSync) flickers/blanks on NVIDIA Wayland; uncomment for single-monitor or uniform-VRR setups. Read by `pam_systemd` at graphical-session start (NOT by `/etc/environment`, which only login(1) / sshd-launched shells see), so the variables reach KWin, Firefox, and the rest of the user session uniformly. |
